...
public sealed class ShipyardConsoleSystem : SharedShipyardConsoleSystem
{
    ...
    protected override void TryPurchase(Entity<ShipyardConsoleComponent> ent, EntityUid user, VesselPrototype vessel)
    {
        ...
        var distribution = _cargo.CreateAccountDistribution(bank.Comp.PrimaryAccount, bank.Comp, bank.Comp.PrimaryCut);
        var bankOptional = (bank.Owner, (StationBankAccountComponent?) bank.Comp);
        _cargo.UpdateBankAccount(bankOptional, -vessel.Price, distribution);
        ...
        UpdateUI(ent, bank.Comp.Accounts[bank.Comp.PrimaryAccount]);
    }
