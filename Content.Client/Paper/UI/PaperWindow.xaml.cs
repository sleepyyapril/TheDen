...
using Content.Shared._DEN.PaperEditor;
...
namespace Content.Client.Paper.UI
{
    ...
    public sealed partial class PaperWindow : BaseWindow
    {
        ...
        {
            var editorToolPrototypes = _prototypeManager.EnumeratePrototypes<EditorTagPrototype>();
            var editorToolButtons = GetEditorToolbarButtons(editorToolPrototypes);
        }

        private Dictionary<ProtoId<EditorTagPrototype>, Button> GetEditorToolbarButtons(IEnumerable<EditorTagPrototype> editorTagPrototypes)
        {
            Dictionary<ProtoId<EditorTagPrototype>, Button> toolButtons = new();

            foreach (var editorTagPrototype in editorTagPrototypes)
            {
                var editorButton = new Button()
                {
                    HorizontalExpand = true,
                    Text = editorTagPrototype.ButtonText,
                    ToolTip = editorTagPrototype.Tooltip
                };

                editorButton.OnPressed += args => HandleEditorButtonPressed(editorTagPrototype, args);
                toolButtons.Add(editorTagPrototype.ID, editorButton);
            }

            return toolButtons;
        }

        private void HandleEditorButtonPressed(EditorTagPrototype editorTagPrototype, BaseButton.ButtonEventArgs args)
        {

        }

        private void CustomInsertAt(TextEdit.CursorPos pos, string text)
        {
            Input.TextRope = Rope.Insert(Input.TextRope, pos.Index, text);
        }
        ...
        public void Populate(SharedPaperComponent.PaperBoundUserInterfaceState state)
        {
            ...
            PaperEditTools.Visible = isEditing;
