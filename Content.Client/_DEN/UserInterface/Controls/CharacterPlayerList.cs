...
public sealed partial class CharacterPlayerList : BoxContainer
{
    private readonly AdminSystem _adminSystem;

    private readonly IEntityManager _entManager;
    private readonly IUserInterfaceManager _uiManager;
    private readonly ConsentSystem _consentSystem;
    ...
    public CharacterPlayerList()
    {
        _entManager = IoCManager.Resolve<IEntityManager>();
        _uiManager = IoCManager.Resolve<IUserInterfaceManager>();
        _consentSystem = _entManager.System<ConsentSystem>();
        _adminSystem = _entManager.System<AdminSystem>();

        RobustXamlLoader.Load(this);

        ListContainer.ItemPressed += PlayerListItemPressed;
        ListContainer.ItemKeyBindDown += PlayerListItemKeyBindDown;
        ListContainer.GenerateItem += GenerateButton;
        ListContainer.NoItemSelected += PlayerListNoItemSelected;
        PopulateList(_adminSystem.PlayerList);
        FilterLineEdit.OnTextChanged += _ => FilterList();
        _adminSystem.PlayerListChanged += PopulateList;
        BackgroundPanel.PanelOverride = new StyleBoxFlat { BackgroundColor = new(32, 32, 40) };
    }
    ...
    private void FilterList()
    {
        ...
        ListContainer.PopulateList(_sortedPlayerList.Select(info => new PlayerListData(info)).ToList());

        if (_selectedPlayer != null)
            ListContainer.Select(new PlayerListData(_selectedPlayer));
    }
    ...
    private void GenerateButton(ListData data, ListContainerButton button)
    {
        ...
        var entry = new CharacterPlayerListEntry();
